...
var cssmw_home = {
	...
	ie6_li_mouseOver: function(e) {
		cssmw_home.clearHoverLIs();
		cssmw_home.setHovFocLI(this, "hoverLI");
		var doBlur = (cssmw_home.focusLI && true);
		cssmw_home.clearFocusLIs();
		if (doBlur) {
			cssmw_home.blurFocus();
			cssmw_home.clearFocus();
		}
		cssmw_home.setLIAndChildNodes(this, "link", "hover");
	...
	ie6_li_mouseOut: function(e) {
		cssmw_home.clearHoverLIs();
		var ul = this.getElementsByTagName('ul')[0];
		cssmw_home.setLIAndChildNodes(this, "hover", "link");
	...
	a_focus: function(e) {
		...
		if (!li) return;
		cssmw_home.clearFocusLIs();
		cssmw_home.setHovFocLI(li, "focusLI");
		cssmw_home.setLIAndChildNodes(li, "link", "hover");
		if (cssmw_home.browser.isIE6) {
			var ul = cssmw_home.localGetElementsByTagName(li, 'ul')[0];
			if(ul) ul.style.display = 'block';
		}
	},
	...
	a_blur: function(e) {
		if (cssmw_home.stopBlur) {
			return;
		}
		if (cssmw_home.focusLI) {
			cssmw_home.remHovFocLI(cssmw_home.focusLI, "focusLI");
		}
		setTimeout("cssmw_home.clearFocus();", 1);
	},
	...
	a_onkeyup: function(e) {
		if (!cssmw_home.orientation) return;
		var key = 0;
		if (window.event) {
		...
		cssmw_home.doKeyPress(key, this);
	},
