...
var cssmw_content = {
	...
	ie6_li_mouseOver: function(e) {
		cssmw_content.clearHoverLIs();
		cssmw_content.setHovFocLI(this, "hoverLI");
		var doBlur = (cssmw_content.focusLI && true);
		cssmw_content.clearFocusLIs();
		if (doBlur) {
			cssmw_content.blurFocus();
			cssmw_content.clearFocus();
		}
		cssmw_content.setLIAndChildNodes(this, "link", "hover");
	...
	ie6_li_mouseOut: function(e) {
		cssmw_content.clearHoverLIs();
		var ul = this.getElementsByTagName('ul')[0];
		cssmw_content.setLIAndChildNodes(this, "hover", "link");
	...
	a_focus: function(e) {
		...
		if (!li) return;
		cssmw_content.clearFocusLIs();
		cssmw_content.setHovFocLI(li, "focusLI");
		cssmw_content.setLIAndChildNodes(li, "link", "hover");
		if (cssmw_content.browser.isIE6) {
			var ul = cssmw_content.localGetElementsByTagName(li, 'ul')[0];
			if(ul) ul.style.display = 'block';
		}
	},
	...
	a_blur: function(e) {
		if (cssmw_content.stopBlur) {
			return;
		}
		if (cssmw_content.focusLI) {
			cssmw_content.remHovFocLI(cssmw_content.focusLI, "focusLI");
		}
		setTimeout("cssmw_content.clearFocus();", 1);
	},
	...
	a_onkeyup: function(e) {
		if (!cssmw_content.orientation) return;
		var key = 0;
		if (window.event) {
		...
		cssmw_content.doKeyPress(key, this);
	},
